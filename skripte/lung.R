...
lung_studies_info <- available_studies() %>%
  filter(str_detect(name, regex("lung", ignore_case = TRUE))) %>%
  select(studyId, name)

lung_studies <- lung_studies_info$studyId

#saving information about study names

lung_studies_df <- as.data.frame(lung_studies)
write_xlsx(lung_studies_df,
           path = "C:/Users/isido/OneDrive/Documents/cbioportal/tabele/lung/lung_studies.xlsx")

# =========================
# 2. Get All Mutations and Add Study Names
# =========================
all_mutations <- map_dfr(lung_studies, function(study_id) {
  tryCatch(get_mutations_by_study(study_id), error = function(e) NULL)
}) %>%
  left_join(lung_studies_info, by = "studyId")

all_clinical <- map_dfr(lung_studies, function(study_id) {
...
write_xlsx(summary_table,
           path = "C:/Users/isido/OneDrive/Documents/cbioportal/tabele/lung/summary_table.xlsx")
...
p <- ggplot(df_overall, aes(x = Category, y = Percentage, fill = Category)) +
  ...
  ylim(0, 100) +
  labs(title = "Mutation percentage in lung samples",
       x = "", y = "Percentage (%)") +
...
# Save the plot
ggsave("grafici/lung/mutation_percentage_lung.png", plot = p, width = 12, height = 14, dpi = 300)
...
p <- ggplot(df_plot_group_sample, aes(x = Aggressiveness, y = Percentage, fill = Category)) +
  ...
  ylim(0, 100) +
  labs(title = "Mutations by sample aggressiveness in lung samples",
...
ggsave("grafici/lung/mutation_percentage_agg_lung.png", plot = p, width = 12, height = 14, dpi = 300)
...
write_xlsx(group_fisher,
           path = "tabele/lung/statistika/group_fisher_lung.xlsx")
...
p <- ggplot(df_aggressive_plot, aes(x = Category, y = Percentage, fill = Category)) +
  ...
  scale_fill_manual(values = c("Mutation" = "#4D4D4D", "No mutation" = "#56B4E9")) +
  ylim(0, 100) +
  labs(title = "Mutation percentage in only aggressive lung samples",
       x = "", y = "Percentage (%)") +
  facet_wrap(~ hugoGeneSymbol) +
  theme_minimal(base_size = 14) +
  theme(
...
ggsave("grafici/lung/mutation_percentage_only_aggressive_lung.png", plot = p, width = 12, height = 14, dpi = 300)
...
write_xlsx(fisher_aggressive,
           path = "tabele/lung/statistika/fisher_aggressive_lung.xlsx")
...
# Save the plot
ggsave("grafici/lung/mutation_percentage_fams_lung.png", plot = p, width = 14, height = 16, dpi = 300)
...
write_xlsx(global_fisher,
           path = "tabele/lung/statistika/famA_fisher_lung.xlsx")
...
write_xlsx(global_fisher,
           path = "tabele/lung/statistika/famB_fisher_lung.xlsx")
...
write_xlsx(global_fisher,
           path = "tabele/lung/statistika/famD_fisher_lung.xlsx")
...
ggsave("grafici/lung/mutation_percentage_agg_fams_lung.png", plot = p, width = 16, height = 18, dpi = 300)
...
write_xlsx(group_fisher,
           path = "tabele/lung/statistika/famA_group_fisher_lung.xlsx")
...
write_xlsx(group_fisher,
           path = "tabele/lung/statistika/famB_group_fisher_lung.xlsx")
...
write_xlsx(group_fisher,
           path = "tabele/lung/statistika/famD_group_fisher_lung.xlsx")
...
# Save the plot
ggsave("grafici/lung/mutation_percentage_aggressive_fams_lung.png", plot = p, width = 14, height = 16, dpi = 300)
...
write_xlsx(fisher_aggressive,
           path = "tabele/lung/statistika/fisher_aggressive_famA_lung.xlsx")
...
write_xlsx(fisher_aggressive,
           path = "tabele/lung/statistika/fisher_aggressive_famB_lung.xlsx")
...
write_xlsx(fisher_aggressive,
           path = "tabele/lung/statistika/fisher_aggressive_famD_lung.xlsx")
