...
prostate_studies_info <- available_studies() %>%
  filter(str_detect(name, regex("prostate", ignore_case = TRUE))) %>%
  select(studyId, name)

prostate_studies <- prostate_studies_info$studyId

#saving information about study names

prostate_studies_df <- as.data.frame(prostate_studies)
write_xlsx(prostate_studies_df,
           path = "C:/Users/isido/OneDrive/Documents/cbioportal/tabele/prostate/prostate_studies.xlsx")

# =========================
# 2. Get All Mutations and Add Study Names
# =========================
all_mutations <- map_dfr(prostate_studies, function(study_id) {
  tryCatch(get_mutations_by_study(study_id), error = function(e) NULL)
}) %>%
  left_join(prostate_studies_info, by = "studyId")

all_clinical <- map_dfr(prostate_studies, function(study_id) {
...
write_xlsx(summary_table,
           path = "C:/Users/isido/OneDrive/Documents/cbioportal/tabele/prostate/summary_table.xlsx")
...
p <- ggplot(df_overall, aes(x = Category, y = Percentage, fill = Category)) +
  ...
  ylim(0, 100) +
  labs(title = "Mutation percentage in prostate samples",
       x = "", y = "Percentage (%)") +
...
# Save the plot
ggsave("grafici/prostate/mutation_percentage_prostate.png", plot = p, width = 12, height = 14, dpi = 300)
...
write_xlsx(global_fisher,
           "global_fisher_prostate.xlsx")

# =========================
# 5. Mutation Percentage by Aggressiveness
# =========================
...
p <- ggplot(df_plot_group_sample, aes(x = Aggressiveness, y = Percentage, fill = Category)) +
  ...
  ylim(0, 100) +
  labs(title = "Mutations by sample aggressiveness in prostate samples",
...
ggsave("grafici/prostate/mutation_percentage_agg_prostate.png", plot = p, width = 12, height = 14, dpi = 300)
...
write_xlsx(group_fisher,
           path = "tabele/prostate/statistika/group_fisher_prostate.xlsx")


# =========================
# 6. Mutation Percentage only Aggressive samples
# =========================
...
df_aggressive_plot <- df_aggressive_plot %>%
  mutate(hugoGeneSymbol = ifelse(hugoGeneSymbol == "TENT5C", "FAM46C", hugoGeneSymbol))

# Create the plot
p <- ggplot(df_aggressive_plot, aes(x = Category, y = Percentage, fill = Category)) +
  geom_col() +
  geom_text(aes(label = paste0(round(Percentage, 2), "%")),
            vjust = -0.5, size = 5) +
  scale_fill_manual(values = c("Mutation" = "#4D4D4D", "No mutation" = "#56B4E9")) +
  ylim(0, 100) +
  labs(title = "Mutation percentage in only aggressive prostate samples",
       x = "", y = "Percentage (%)") +
  facet_wrap(~ hugoGeneSymbol) +
  theme_minimal(base_size = 14) +
  theme(
...
ggsave("grafici/prostate/mutation_percentage_only_aggressive_prostate.png", plot = p, width = 12, height = 14, dpi = 300)
...
write_xlsx(fisher_aggressive,
           path = "tabele/prostate/statistika/fisher_aggressive_prostate.xlsx")
...
# Save the plot
ggsave("grafici/prostate/mutation_percentage_fams_prostate.png", plot = p, width = 14, height = 16, dpi = 300)
...
write_xlsx(global_fisher,
           path = "tabele/prostate/statistika/famA_fisher_prostate.xlsx")
...
write_xlsx(global_fisher,
           path = "tabele/prostate/statistika/famB_fisher_prostate.xlsx")
...
write_xlsx(global_fisher,
           path = "tabele/prostate/statistika/famD_fisher_prostate.xlsx")
...
ggsave("grafici/prostate/mutation_percentage_agg_fams_prostate.png", plot = p, width = 16, height = 18, dpi = 300)
...
write_xlsx(group_fisher,
           path = "tabele/prostate/statistika/famA_group_fisher_prostate.xlsx")
...
write_xlsx(group_fisher,
           path = "tabele/prostate/statistika/famB_group_fisher_prostate.xlsx")
...
write_xlsx(group_fisher,
           path = "tabele/prostate/statistika/famD_group_fisher_prostate.xlsx")

# =========================
# 9. Mutation Percentage only Aggressive samples (sample-level), FAM46 family
# =========================
...
# Save the plot
ggsave("grafici/prostate/mutation_percentage_aggressive_fams_prostate.png", plot = p, width = 14, height = 16, dpi = 300)
...
write_xlsx(fisher_aggressive,
           path = "tabele/prostate/statistika/fisher_aggressive_famA_prostate.xlsx")
...
write_xlsx(fisher_aggressive,
           path = "tabele/prostate/statistika/fisher_aggressive_famB_prostate.xlsx")
...
write_xlsx(fisher_aggressive,
           path = "tabele/prostate/statistika/fisher_aggressive_famD_prostate.xlsx")
